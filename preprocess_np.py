...
def generate_parquet(feature_path, mask_path, output_path):
    ...
    image_flat_numpy_rdd = images_rdd.map(lambda pair_raw_image_vs_path: (raw_image_to_numpy_array(pair_raw_image_vs_path[1]), pair_raw_image_vs_path[0])) \
                                              .map(lambda np_array_vs_path: {'features': np_array_vs_path[0], 'img_name': np_array_vs_path[1].split('/')[-1]}) \
    ...
    mask_flat_numpy_rdd = mask_rdd.map(lambda pair_raw_image_vs_path: (raw_image_to_numpy_array(pair_raw_image_vs_path[1]), pair_raw_image_vs_path[0])) \
                                           .map(lambda np_array_vs_path: ((np_array_vs_path[0] / 255).astype(np.uint8), np_array_vs_path[1].split('/')[-1])) \
                                           .map(lambda pair_std_np_array_imgname: {'masks': pair_std_np_array_imgname[0], 'img_name': pair_std_np_array_imgname[1]}) \
    ...
    train_df = image_df.join(mask_df, "img_name", "inner").drop('img_name')
    #print("Summary =>>>>>>>>>>>>>>>>>>>>>>>....>>>")
